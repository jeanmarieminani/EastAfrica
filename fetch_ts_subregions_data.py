...
def request_power_data_csv(lat_min, lat_max, lon_min, lon_max, start_year, end_year, file_name):
    """Request NASA POWER data for TS (Earth Skin Temperature) and save it as CSV."""
    url = "https://power.larc.nasa.gov/api/temporal/monthly/regional"
    params = {
        "parameters": "TS",             # Earth Skin Temperature
    ...
    try:
        print(f"Requesting TS data for {file_name}")
...
def main():
    ...
    for i, region in enumerate(sub_regions):
        file_name = os.path.join(
            output_dir,
            f"region_{i+1}_TS_{start_year}_{end_year}.csv"
        )
